...
from models.superresolution import Interpolation, SR_Module
...
def reconstruction(args):
    ...
    # grad_vars = [{'params':sr_module.parameters(), 'lr':0.01}]

    # freeze tensorf from backprop
    ...
    # pbar = tqdm(range(args.n_iters, args.n_iters+args.n_iters_sr), miniters=args.progress_refresh_rate, file=sys.stdout)
    pbar = tqdm(range(args.n_iters), miniters=args.progress_refresh_rate, file=sys.stdout)
    for iteration in pbar:
        ...
        # rgb_map, alphas_map, depth_map, weights, uncertainty
        rgb_map, sr_map, alphas_map, depth_map, weights, uncertainty = renderer_sr(ds_rays, cam_params, tensorf, sr_module, img_wh=(ds_W, ds_H), chunk=args.batch_size,
                                N_samples=nSamples, white_bg = white_bg, ndc_ray=ndc_ray, device=device, is_train=True)
        ...
        # loss
        # total_loss = 0.1*nerf_loss + 0.9*sr_loss
        total_loss = sr_loss
        if Ortho_reg_weight > 0:
            loss_reg = tensorf.vector_comp_diffs()
            total_loss += Ortho_reg_weight*loss_reg
